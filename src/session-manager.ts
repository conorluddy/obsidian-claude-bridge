 ...
 *
 * Each note can have at most one active session. The session ID
 * is stored in the note's YAML frontmatter as `claude_session`.
 */
export class SessionManager {
	...
	getSessionId(file: TFile): string | null {
		...
		this.logger.debug(
			"session",
			`getSessionId("${file.path}") → ${sessionId ?? "(none)"}`,
		);
		return sessionId;
	}
	...
	async setSessionId(file: TFile, sessionId: string): Promise<void> {
		this.logger.debug(
			"session",
			`setSessionId("${file.path}", "${sessionId}")`,
		);
